...
class Builder
{
    ...
    /**
     * The "with" that should be applied to the search.
     *
     * @var string|array
     */
    public $with;
     ...
     */
    public function paginate($perPage = null, $page = null, $simple = false, $pageName = 'page')
    {
        $engine = $this->engine();
        ...
        $config = $this->model->getPaginateConfig();

        $paginatorDriver = $this->model->getPaginatorDriver();

        $paginator = (new $paginatorDriver($results, $perPage, $page, $engine->getTotalCount($rawResults), $simple, array_merge($config, [
            'path' => call_user_func([$paginatorDriver, 'getCurrentPath']),
            'var_page' => $pageName,
        ])));
