...
use brooke\scout\jobs\MakeSearchable;
use brooke\supports\ModelCollection;
use brooke\scout\events\ModelsFlushed;
...
trait Searchable
{
    ...
    public static function init()
    {
        ...
        Query::extend('unsearchable', function (Query $query, $chunk = null) {
            $query->chunkById($chunk ?: config('scout.chunk.unsearchable', 500), function ($models) {
                $models->unsearchable();
                Hook::listen(ModelsFlushed::class, $models);
            });
        });

        Query::extend('searchable', function(Query $query, $chunk = null){
            $query->chunkById($chunk ?: config('scout.chunk.searchable', 500), function ($models) {
                 $models->filter->shouldBeSearchable()->searchable();
                 Hook::listen(ModelsImported::class, $models);
            });
        });
    ...
    public function registerSearchableMacros()
    {
        ...
        ModelCollection::macro('searchable', function () use ($self) {
            $self->queueMakeSearchable($this);
        });

        ModelCollection::macro('unsearchable', function () use ($self) {
